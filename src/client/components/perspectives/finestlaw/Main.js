...
const Main = (props) => {
  ...
  return (
    <Box
      sx={(theme) => {
        const defaultHeightReduction =
          layoutConfig.topBar.defaultHeight +
          layoutConfig.footer.defaultHeight +
          getSpacing(theme, 2);
        const reducedHeightReduction =
          layoutConfig.topBar.reducedHeight +
          layoutConfig.footer.reducedHeight +
          getSpacing(theme, 2);
        return {
    ...
    >
      ...
      >
        ...
        <Grid
          container
          spacing={screenSize === "sm" ? 2 : 1}
          justifyContent={
            screenSize === "xs" || screenSize === "sm" ? "center" : "flex-start"
          }
        >
          {perspectives.map((perspective) => {
            const hideCard =
              has(perspective.hideCardOnFrontPage) &&
              perspective.hideCardOnFrontPage;
            if (!hideCard) {
              return (
                <MainCard
                  key={perspective.id}
