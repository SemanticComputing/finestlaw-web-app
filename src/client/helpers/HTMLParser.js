...
export default class HTMLParser {
  ...
  parseHTML(html) {
    ...
    switch (this.props.HTMLParserTask) {
      ...
      case "addAnnotationTooltips":
        ...
        preprocessNodes = this.preprocessNodes.bind(this);
        break;
      default:
        transform = null;
    }
  ...
  preprocessNodes(nodes) {
    ...
    const getChapterNumber = (chapterNumber) => {
      let number
      if (this.props?.langTag === "en") {
        number = chapterNumber.replace("chapter ", "")
      } else if (this.props.langTag === "et") {
        number = chapterNumber.replace(". peatükk", "").replace(" peatükk", "")
      } else {
        number = chapterNumber.replace(" luku", "")
      }
      return number
    }
    // Add new divs with ids for each section
    const sectionDivs = findAll(
      (node) => node.attribs.class === "section",
      nodes
    );
    sectionDivs.forEach((node) => {
      ...
      if (
        node.parent.attribs.class === "entry-into-force" &&
        node.parent.parent.attribs.class === "chapter"
      ) {
        chapterNumber = node.parent.parent.children[0].children[0].data;
        chapterNumber = `chapter_${getChapterNumber(chapterNumber)}_`;
        chapterNumber = chapterNumber.replace(/\s/g, "");
      } else if (node.parent.attribs.class === "chapter") {
        chapterNumber =
          node.parent.children[0]?.children[0]?.data.toLowerCase() ?? "";
        chapterNumber = `chapter_${getChapterNumber(chapterNumber)}_`;
        chapterNumber = chapterNumber.replace(/\s/g, "");
      } else {
        chapterNumber = "";
      ...
      if (sectionItemIdentifier.includes("§")) {
        sectionNumber = sectionItemIdentifier
          .replace(/\s/g, "")
          .replace("§", "");
      } else {
        sectionNumber = "1";
  ...
  processReferencedTerms = () => {
    ...
    if (Array.isArray(referencedTerm)) {
      this.referencedTermsObj = arrayToObject({
        array: referencedTerm,
        keyField: "id",
      });
    }
  };
  ...
  addAnnotationTooltips = (node, index) => {
    ...
    if (
      ...
      return (
        <div
          ...
          className="ref"
          ref={(element) => {
            this.props.sectionRefs.current[id] = element;
          }}
        />
      );
    }
    ...
    if (
      this.referencedTermsObj &&
      node.type === "tag" &&
      node.name === "span" &&
      node.attribs.name === "namedentity" &&
      node.attribs["data-link"] &&
      node.attribs["data-link"] !== ""
    ) {
      ...
      if (linkStr.includes(",")) {
        ...
        tooltipJSX = (
          <div className={props.classes.tooltipContent}>{urisJSX}</div>
        );
      } else {
        const uri = linkStr;
        if (!this.shouldAddAnnotation(uri)) {
          return;
        }
        tooltipJSX = (
  ...
  shouldAddAnnotation = (uri) => {
    ...
    return (
      (uri.startsWith("http://fi.dbpedia.org/") ||
        uri.startsWith("http://ldf.fi/ttp/")) &&
      this.referencedTermsObj[uri] &&
      this.referencedTermsObj[uri].externalLink
    );
  };
  ...
  renderAnnotation = (uri) => {
    ...
    const { prefLabel, description, externalLink } =
      this.referencedTermsObj[uri];
    let source;
    if (uri.startsWith("http://ldf.fi/ttp/")) {
